...
monte.carlo.lca <-
  function(product.list) {
    ...
    subcats.keep <- 
      read.csv(paste0(getwd(),'/Data Inputs/Search words, second round, 2024.07.22.csv')) %>%
    ...
    cat(nrow(tmp.food.df),'\n')
    
    # looping across foods
    # for(i in product.list[1:50]) { # for testing
    for(i in product.list) {
      ...
      if(nrow(tmp.lca.sample)>1) { # If there's an observation
        tmp.lca.sample <-
          ...
          dplyr::sample_n(n_rep, weight = Weight, replace = TRUE) %>%
          .[order(.$Food_Category),] %>%
          as.data.frame(.) %>%
          mutate(sample_index = rep(1:n_rep, length(unique(paste0(.$Food_Category,.$Food_Category_sub,.$Food_Category_sub_sub)))))
      ...
      # Creating template
      mean.se.ci.out <- data.frame(matrix(nrow = n_rep, ncol = (3 + length(indicators))))
      # Names
      ...
      mean.se.ci.out[,grepl('tmp.impact_[0-9]',names(mean.se.ci.out))] <- 
        rowsum(tmp.food[!is.na(tmp.food$sample_index),grepl('tmp.impact_[0-9]',names(tmp.food))],
               group = tmp.food$sample_index[!is.na(tmp.food$sample_index)])
      
      # Scaling
      # This is only done based on the specified indicators
      # Need to do this in a loop
      for(k in which(indicators %in% indicators.scale)) {
        mean.se.ci.out[,paste0('scaled_',k)] <-
          mean.se.ci.out[,paste0('tmp.impact_',k)] /
          max(lca.scaling[,paste0('impact_',k)])
      }
      
      # And scaling
      mean.se.ci.out$tot_scaled <-
        base::rowSums(mean.se.ci.out[,paste0('scaled_',which(indicators %in% indicators.scale))]) /
        length(indicators.scale)
      
      # Ordering mean.se.ci first
      mean.se.ci.out <- mean.se.ci.out[order(mean.se.ci.out$tot_scaled),]
      
      # Getting min and max
      # Df is sorted, so just need to take first and last row
      min.food <- mean.se.ci.out[1,c('product_name','id',paste0('tmp.impact_',1:length(indicators)))]
      names(min.food) <- gsub('tmp.impact_','min.impact_',names(min.food))
      
      max.food <- mean.se.ci.out[n_rep,c('product_name','id',paste0('tmp.impact_',1:length(indicators)))]
      names(max.food) <- gsub('tmp.impact_','max.impact_',names(max.food))
      
      # 5th and 95th quantile
      lower.fifth.food <- mean.se.ci.out[fifth.obs,c('product_name','id',paste0('tmp.impact_',1:length(indicators)))]
      names(lower.fifth.food) <- gsub('tmp.impact_','lower_fifth.impact_',names(lower.fifth.food))
      
      upper.ninety.fifth.food <- mean.se.ci.out[ninetyfifth.obs,c('product_name','id',paste0('tmp.impact_',1:length(indicators)))]
      names(upper.ninety.fifth.food) <- gsub('tmp.impact_','upper_ninetyfifth.impact_',names(upper.ninety.fifth.food))
      
      lower.twentyfifth.food <- mean.se.ci.out[twentyfifth.obs,c('product_name','id',paste0('tmp.impact_',1:length(indicators)))]
      names(lower.twentyfifth.food) <- gsub('tmp.impact_','lower_twentyfifth.impact_',names(lower.twentyfifth.food))
      
      fifty.food <- mean.se.ci.out[fifty.obs,c('product_name','id',paste0('tmp.impact_',1:length(indicators)))]
      names(fifty.food) <- gsub('tmp.impact_','fifty.impact_',names(fifty.food))
      
      upper.seventyfifth.food <- mean.se.ci.out[seventyfifth.obs,c('product_name','id',paste0('tmp.impact_',1:length(indicators)))]
      names(upper.seventyfifth.food) <- gsub('tmp.impact_','upper_seventyfifth.impact_',names(upper.seventyfifth.food))
      
      lower.tenth.food <- mean.se.ci.out[tenth.obs,c('product_name','id',paste0('tmp.impact_',1:length(indicators)))]
      names(lower.tenth.food) <- gsub('tmp.impact_','lower_tenth.impact_',names(lower.tenth.food))
      
      upper.ninety.food <- mean.se.ci.out[ninety.obs,c('product_name','id',paste0('tmp.impact_',1:length(indicators)))]
      names(upper.ninety.food) <- gsub('tmp.impact_','upper_ninety.impact_',names(upper.ninety.food))
      # And getting mean across all monte carlo runs
      
      mean.se.ci.mean <- colMeans(as.matrix(mean.se.ci.out[,paste0('tmp.impact_',1:length(indicators))]),na.rm=TRUE)
      mean.se.ci.se <- colSds(as.matrix(mean.se.ci.out[,paste0('tmp.impact_',1:length(indicators))]),na.rm=TRUE) / sqrt(n_rep)
      
      mean.se.ci.out <- data.frame(product_name = unique(mean.se.ci.out$product_name),
                                   id = unique(mean.se.ci.out$id))
      
      mean.se.ci.out[,paste0('mean_',1:length(indicators))] <- mean.se.ci.mean
      mean.se.ci.out[,paste0('se_',1:length(indicators))] <- mean.se.ci.se
      
      
      # adding confidence intervals
      # lower ci
      mean.se.ci.out[,paste0('lower_ci_',1:length(indicators))] <-
        mean.se.ci.out[,grepl(paste0("mean_"),names(mean.se.ci.out))] -
        qt(1 - (0.05 / 2), n_rep - 1) * mean.se.ci.out[,grepl(paste0("se_"), names(mean.se.ci.out))]
      # upper ci
      mean.se.ci.out[,paste0('upper_ci_',1:length(indicators))] <-
        mean.se.ci.out[,grepl(paste0("mean_"),names(mean.se.ci.out))] +
        qt(1 - (0.05 / 2), n_rep - 1) * mean.se.ci.out[,grepl(paste0("se_"), names(mean.se.ci.out))]
      ...
      merge.in <-
        ...
        left_join(.,lower.tenth.food) %>%
        left_join(.,lower.twentyfifth.food) %>%
        left_join(.,fifty.food) %>%
        left_join(.,upper.seventyfifth.food) %>%
        left_join(.,upper.ninety.food) %>%
        left_join(.,upper.ninety.fifth.food)
      
      
      mean.se.ci <-
        left_join(mean.se.ci.out,merge.in)
      
      # Merging in the check for number of lca samples
      mean.se.ci <-
        cbind(mean.se.ci,min(tmp.food.check$count),max(tmp.food.check$count))
      
      # And adding to data frame
      out.df <- rbind(out.df, mean.se.ci)
    }
