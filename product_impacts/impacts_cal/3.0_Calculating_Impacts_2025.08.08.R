...
# Loading functions
source('/Users/shruti/OneDrive - Nexus365/DPhil/code/food_product_impacts/product_impacts/impacts_cal/0.0_Functions_Estimating_Impacts_2025.08.08.R')
...
file.list = list.files(path = paste0(getwd(),'/Outputs'),
                       pattern = 'percent composition by ingredient EUwest', full.names = TRUE) #%>%
  #.[grepl(Sys.Date(),.)]
...
#           paste0(getwd(),"/Managed_Data/NutriScore for radar plots EAsia ",Sys.Date()," Log2.csv"),
#           row.names = FALSE)
...
for(c in (countries)) {
  ...
  # if (c %in% c('France')) {
    ...
    chunk.size <- 1000 # could increase to about 5k maybe
