...
# ingredient_lapply_function <-
  ...
  file_check <- 
    ...
    # .[grepl(Sys.Date(),.)] %>%
    .[grepl(retail, .) & grepl("percent provided", ., ignore.case = TRUE)] %>%
  ...
  if(max(loops) >= 1) { # if else statement to loop across departments, if there are departments left to loop across
    ...
    for(z in 1:loops) {
      ...
      # Identifying ingredients where composition is provided
      # Identifiers are product id, product name, and variablen (order of ingredient in the product)
      dat.long <-
        dat.long %>%
      ...
      percent_known_all <-
        ...
        dplyr::select(id, product_name, Retailer, country, Department, Aisle, Shelf,
                      variable, percent_not_embedded = percent_updated, percent_not_embedded_provided = percent_provided)
      ...
      if(nrow(dat.long) >= 1 &
        ...
        percent_known_embedded <-
          dat.long %>%
          mutate(variable_embedded = paste0(variable,'.',variable_embedded)) %>%
          dplyr::select(id, product_name, Retailer, country, Department, Aisle, Shelf,
                        variable, variable_embedded, percent_embedded = percent_numeric) %>%
          mutate(percent_embedded_provided = !is.na(percent_embedded))
        
        # Merging in
        percent_known_all <- 
          full_join(percent_known_all %>%
                      distinct(), 
                    percent_known_embedded %>%
                      distinct())
      }
      
      # If else check to add column names
      if('percent_embedded_provided' %in% names(percent_known_all)) {
        # Do nothing
      } else {
        # Add empty columns
        percent_known_all <-
          ...
          mutate(variable_embedded = NA,
                 percent_embedded = NA,
                 percent_embedded_provided = NA)
      }
      ###
      # Saving data frame
      write.csv(percent_known_all,
                paste0(getwd(),"/Outputs/percent provided by ingredient ",
