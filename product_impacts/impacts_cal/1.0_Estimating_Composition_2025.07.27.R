...
# ingredient_lapply_function <-
  ...
  file_check <- 
    ...
    # .[grepl(Sys.Date(),.)] %>%
    .[grepl(retail, .) & grepl("percent composition", ., ignore.case = TRUE)] %>%
  ...
  if(max(loops) >= 1) { # if else statement to loop across departments, if there are departments left to loop across
    ...
    for(z in 1:loops) {
      ...
      dat.long <- 
        ...
        mutate(percent_provided = !is.na(percent_updated))
      ...
      dat <-
        ...
        mutate(flour_minerals =
                 grepl('flour',value,ignore.case=TRUE) &
                 grepl('with|iron|calcium|carbonate|niacin|thiamin|acid|nicotinic|added|fortified',embedded_ingredients,ignore.case=TRUE)) %>%
        filter(!flour_minerals) %>%
        dplyr::select(-flour_minerals) %>%
      ...
      if(nrow(dat.long) >= 1 &
        ...
        dat.long <-
          dat.long %>%
          mutate(percent_provided = !is.na(percent_numeric))
