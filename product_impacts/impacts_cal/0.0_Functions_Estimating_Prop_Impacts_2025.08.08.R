...
monte.carlo.lca <-
  function(product.list) {
    ...
    for(i in product.list) {
      ...
      tmp.food_no_nas <-
        tmp.food %>%
        filter(!is.na(tmp.impact_1))
      
      # getting proportional impacts
      tmp_food_sum <-
        tmp.food %>%
        filter(!is.na(tmp.impact_1)) %>%
        dplyr::group_by(product_name, id, Food_Category, Retailer, Department, Aisle, Shelf) %>%
        dplyr::summarise(tmp.impact_1 = sum(tmp.impact_1) / sum(tmp.food_no_nas$tmp.impact_1),
                         tmp.impact_2 = sum(tmp.impact_2) / sum(tmp.food_no_nas$tmp.impact_2),
                         tmp.impact_3 = sum(tmp.impact_3) / sum(tmp.food_no_nas$tmp.impact_3),
                         tmp.impact_4 = sum(tmp.impact_4) / sum(tmp.food_no_nas$tmp.impact_4),
                         tmp.impact_5 = sum(tmp.impact_5) / sum(tmp.food_no_nas$tmp.impact_5),
                         tmp.impact_6 = sum(tmp.impact_6) / sum(tmp.food_no_nas$tmp.impact_6),
                         tmp.impact_7 = sum(tmp.impact_7) / sum(tmp.food_no_nas$tmp.impact_7),
                         amount = sum(amount) / sum(tmp.food_no_nas$amount))
      
      # And adding to data frame
      out.df <- rbind(out.df, tmp_food_sum)
      
      rm(tmp_food_sum,tmp.food,tmp.food_no_nas)
    }
